 ...
 */
public class WiresharkProgrammingTask {
	...
	public static void main(String[] args) {
		 ...
		 * annonymous handler which will receive packets as they are read from
		 * the offline file by libpcap. We parameterize it with a StringBuilder
		 * class. This allows us to pass in any type of object we need inside
		 * the our dispatch handler. For this example we are passing in the
		 * errorbuf object so we can pass back a string, if we need to. Of
		 * course in our example this is not strictly needed since our anonymous
		 * class can access errbuf object directly from the enclosing main
		 * method as that local variable is marked final allowing anonymous
		...
		pcap.loop(10, new JPacketHandler<StringBuilder>() {
			 ...
			 * @param errbuf
			 *            our custom user parameter which we chose to be a
			 *            StringBuilder object, but could have chosen anything
			 *            else we wanted passed into our handler by libpcap
			 */
			public void nextPacket(JPacket packet, StringBuilder errbuf) {

				/*
				 * Here we receive 1 packet at a time from the capture file. We
				 * are going to check if we have a tcp packet and do something
				 * with tcp header. We are actually going to do this twice to
				 * show 2 different ways how we can check if a particular header
				 * exists in the packet and then get that header (peer header
				 * definition instance with memory in the packet) in 2 separate
				 * steps.
				 */
				if (packet.hasHeader(Tcp.ID)) {

					/*
					 * Now get our tcp header definition (accessor) peered with
					 * actual memory that holds the tcp header within the
					 * packet.
					 */
				...
				/*
				 * An easier way of checking if header exists and peering with
				 * memory can be done using a conveniece method
				 * JPacket.hasHeader(? extends JHeader). This method performs
				 * both operations at once returning a boolean true or false.
				 * True means that header exists in the packet and our tcp
				 * header difinition object is peered or false if the header
		...
		for (int i = 0; i < 50; i++) {
			 ...
			 * present in the map or not and to retrieve values. jNetPcap
			 * provides us with a special object called a JFlow which keeps a
			 * list of packets part of that flow. We can add new packets to a
			 * flow and later we can get a list of those packets. So first we
			 * check if a flow for a given key already exists. All packets part
			 * of the same flow will have the same key.
			 */
			...
			/*
			 * Now that we know for sure we have a flow this packet belongs to,
			 * we can add this packet to this flow. Before we can actuall add a
			 * packet to a queue for later processing, we must first make a copy
			 * of the packet to a new object. We can only process each libpcap
			 * packet immediately before any other calls or nextEx or another
			 * iteration of a loop. The packets are delivered to us without
			 * copies so what we are working with is the data within libpcap
			 * buffer. If we want to preserve a packet beyond this point, we
			 * have to make a copy of the packet and its decoded state and then
			 * we can keep the packet around for as long as its needed. There is
			 * a convenience PcapPacket constructor that does a copy of
		...
		for (JFlow flow : flows.values()) {
			...
			/*
			 * Flows can be bi-directional. That is packets going between host A
			 * and B would be considered in forward-direction, while packets
			 * between host B and A can be considered reserverse direction.
			 * Although both forward and reverse are going in the opposite
			 * directions, jnetpcap flows consider them the same flows. You have
			 * 3 types of accessors for retrieving packets from a flow.
			 * JFlow.getForward, JFlow.getReverse or JFlow.getAll. JFlow.getAll
			 * gets a list of packets, no matter which direction they are going,
			 * while the other 2 accessors only get the packets that are going
			 * in the specified direction.
			 */
			if (flow.isReversable()) {
				/*
				 * We can get directional flow packets, but only if the flow is
				 * reversable. Not all flows are reversable and this is
				 * determined by the header types. If a flow is not reversable,
				 * flow.getReverse will return empty list, which is something we
				 * don't want to have to process.
				 */
				 ...
				 * forward/reverse direction associated with the packets. Here
				 * is how we can do this a little more compactly.
				 */
